...
from features.create_functions import create_careneeder, create_caretaker
...
async def create_profile(req: CreateProfileRequest, user_id: str = Depends(auth.authenticate)):
    if req.role == "careneeder":
        create_careneeder(user_id, req.first_name, req.last_name, req.pfp)
